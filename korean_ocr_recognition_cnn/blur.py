...
#            drawing=cv2.filter2D(drawing,-1,kernel)
...
# Default data paths.
DEFAULT_LABEL_FILE = os.path.join(SCRIPT_PATH,'C:/Users/stork/Desktop/hangul/tensorflow-hangul-recognition-master/labels/4-common-hangul.txt')
DEFAULT_FONTS_DIR = os.path.join(SCRIPT_PATH, 'C:/Users/stork/Desktop/hangul/tensorflow-hangul-recognition-master/randomfont20')
DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_PATH, 'C:/Users/stork/Desktop/hangul/tensorflow-hangul-recognition-master/blurtest')
...
def blur_for_image(image):
    arr = numpy.array(image)

    ##kernel = np.ones((5,5),np.float32)/25##1/25필터 생성
    ##image = cv2.filter2D(arr,-1,kernel)## 1/25 필터 적용
    image = cv2.GaussianBlur(arr, (5, 5), 0)
    image = Image.fromarray(image)
    return image
def blur_for_arr(arr):

    ##kernel = np.ones((5,5),np.float32)/25##1/25필터 생성
    ##image = cv2.filter2D(arr,-1,kernel)## 1/25 필터 적용
    image = cv2.GaussianBlur(arr, (5, 5), 0)###가우시안 blur
