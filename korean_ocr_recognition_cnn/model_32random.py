...
def main(label_file, tfrecords_dir, model_output_dir, num_train_epochs):
    ...
    conv1 = io.open(os.path.join(model_output_dir, 'Conv1.csv'), 'w',
        ...
        try:
            ...
            while True:
                ...
                if step % 100 == 0:
                    ...
                    conv = sess.run(W_conv4, feed_dict={x: train_images,
                                                        y_: train_labels})
                    print(conv)
                    conv = np.reshape(conv, (1, 1, 1, 524288))
                    st=np.array2string(conv)

                    f = open("C:/Users/stork/Desktop/hangul/새파일4.txt", 'w')
